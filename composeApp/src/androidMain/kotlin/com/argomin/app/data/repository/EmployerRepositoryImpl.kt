...
import com.argomin.app.data.local.dao.EmployerDao
...
class EmployerRepositoryImpl(
    ...
    override suspend fun insert(employer: EmployerEntity): Long {
        ...
        val hashedEmployer = employer.copy(
            pin = com.argomin.app.util.PinHasher.hashPin(employer.pin)
        )
                ...
                line?.let {
                    ...
                    if (parts.size >= 4) {
                        val employer = EmployerEntity(
                            ...
                            pin = com.argomin.app.util.PinHasher.hashPin(parts[3].trim()) // Hash PIN from CSV
                        )
